...
public class MinifyAndGzipResponse extends Controller {
    ...
    static {
        ...
        if (Play.mode.isDev() && moduleEnabled) {
            moduleEnabled = !"true".equals(Play.configuration
                    .getProperty("minifymod.moduleDisabledOnDev"));
        }
    }
    ...
    static void compress() throws IOException {
        if (moduleEnabled && !isExcluded() && response != null) {
            ...
            if (minifyEnabled && !isExcluded() && response.contentType != null) {
                ...
                if (response.contentType.contains("text/html")) { // could be
    ...
    /**
     * tells if the client supports gzip. Delegates to Compression.class just
     * here for easy overriding
     */
    ...
    /**
     * tells if the current action is an excluded action. Delegates to
     * Compression.class just here for easy overriding
     */
