...
public class StaticGzipPlugin extends PlayPlugin {
    ...
    @Override
    public boolean serveStatic(final VirtualFile file, final Http.Request request,
        ...
        try {
            ...
            if (memcacheDisabled) { // don't look in cache is disabled
                content = this.minify(request, response, localFile);
            } else {
    ...
    private String minify(final Http.Request request, final Http.Response response, final File file)
            throws IOException {
    ...
    private static Http.Response addEtag(final Http.Request request, final Http.Response response,
            final File file) {
